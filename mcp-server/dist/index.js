 ...
 */
async function executeCommandAsTool(command) {
...
const utilsPath = (script) => path.join(extensionPath, 'utils', script);
server.tool('get_screen', 'Get the current screen state of the Android device as JSON.', {}, () => executeCommandAsTool(`python3 ${utilsPath('get_screen.py')}`));
server.tool('get_screen_summary', 'Get a summarized version of the screen state. Faster and uses fewer tokens. Recommended for initial exploration.', {}, () => executeCommandAsTool(`python3 ${utilsPath('get_screen_summary.py')}`));
server.tool('execute_action', 'Execute an action on the Android device.', {
...
server.tool('check_env', 'Check the ADB environment and Android device connection.', {}, () => executeCommandAsTool(`python3 ${utilsPath('check_env.py')}`));
server.tool('run_ai_script', 'Execute a Python script for complex ADB logic. Use this for multi-step actions to avoid round-trip latency. Available functions: click(text/id/point), type(text, enter=True), wait(seconds), wait_for(text, timeout), home(), back(), find(text/id).', {
    code: z.string().describe('Python code to execute.'),
}, ({ code }) => {
    const encodedCode = Buffer.from(code, 'utf8').toString('base64');
    return executeCommandAsTool(`echo '${encodedCode}' | base64 -d | python3 ${utilsPath('run_ai_script.py')}`);
});
server.tool('execute_batch', 'Execute a sequence of ADB actions in one go. Useful for simple multi-step tasks like filling a form.', {
 ...
 */
function formatDeviceArg(device) {
    return device ? `-s ${device} ` : '';
}
server.tool('adb_devices', 'Lists all connected Android devices and emulators with their status and details.', {}, () => executeCommandAsTool('adb devices -l'));
server.tool('inspect_ui', 'Captures the complete UI hierarchy of the current screen as an XML document. Essential for UI automation and identifying interactive elements.', {
...
server.tool('adb_logcat', 'Retrieves Android system and application logs from a connected device.', {
    device: z.string().optional().describe('Optional device ID'),
    ...
    if (fs.existsSync(commandsDir)) {
        ...
        for (const file of files) {
            ...
            while ((match = paramRegex.exec(prompt)) !== null) {
                ...
                if (clean)
                    paramMap.set(clean, { original, clean });
            }
            const shape = {};
            ...
            server.tool(toolName, description, shape, (args) => {
