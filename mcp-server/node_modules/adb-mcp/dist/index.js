...
const unlinkPromise = (0, util_1.promisify)(fs_1.unlink);
const readFilePromise = (0, util_1.promisify)(fs_1.readFile);
// ========== Tool Descriptions ==========
/**
 * Tool description for adb-devices
 */
const ADB_DEVICES_TOOL_DESCRIPTION = "Lists all connected Android devices and emulators with their status and details. " +
 ...
 */
const INSPECT_UI_TOOL_DESCRIPTION = "Captures the complete UI hierarchy of the current screen as an XML document. " +
 ...
 */
const ADB_SHELL_TOOL_DESCRIPTION = "Executes a shell command on a connected Android device or emulator. " +
 ...
 */
const ADB_INSTALL_TOOL_DESCRIPTION = "Installs an Android application (APK) on a connected device or emulator. " +
 ...
 */
const ADB_LOGCAT_TOOL_DESCRIPTION = "Retrieves Android system and application logs from a connected device. " +
 ...
 */
const ADB_PULL_TOOL_DESCRIPTION = "Transfers a file from a connected Android device to the server. " +
 ...
 */
const ADB_PUSH_TOOL_DESCRIPTION = "Transfers a file from the server to a connected Android device. " +
 ...
 */
const ADB_DUMP_IMAGE_TOOL_DESCRIPTION = "Captures the current screen of a connected Android device. " +
 ...
 */
async function executeAdbCommand(command, errorMessage) {
    try {
 ...
 */
function createTempFilePath(prefix, filename) {
    return (0, path_1.join)((0, os_1.tmpdir)(), `${prefix}-${Date.now()}-${(0, path_1.basename)(filename)}`);
}
/**
...
server.resource("adb-version", "adb://version", async (uri) => {
    try {
...
server.resource("device-list", "adb://devices", async (uri) => {
    try {
...
server.tool("adb_devices", types_1.AdbDevicesSchema.shape, async (_args, _extra) => {
...
server.tool("inspect_ui", types_1.AdbUidumpSchema.shape, async (args, _extra) => {
    ...
    try {
        ...
        if (args.asBase64 !== false) {
            ...
            return {
                content: [{ type: "text", text: base64Xml }]
            };
        }
        else {
            ...
            return {
                content: [{ type: "text", text: xmlData }]
            };
        }
    }
    catch (error) {
        ...
        return {
            content: [{ type: "text", text: `Error dumping UI hierarchy: ${errorMsg}` }],
            isError: true
        };
    }
    finally {
...
// Add adb shell tool
server.tool("adb_shell", types_1.AdbShellSchema.shape, async (args, _extra) => {
...
server.tool("adb_install", types_1.AdbInstallSchema.shape, async (args, _extra) => {
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `Error installing APK: ${errorMsg}` }],
            isError: true
        };
    }
}, { description: ADB_INSTALL_TOOL_DESCRIPTION });
// Add adb logcat tool
server.tool("adb_logcat", types_1.AdbLogcatSchema.shape, async (args, _extra) => {
...
// Add adb pull tool
server.tool("adb_pull", types_1.AdbPullSchema.shape, async (args, _extra) => {
    log(LogLevel.INFO, `Pulling file from device: ${args.remotePath}`);
    const deviceArg = formatDeviceArg(args.device);
    const tempFilePath = createTempFilePath("adb-mcp", (0, path_1.basename)(args.remotePath));
    try {
        ...
        if (args.asBase64 !== false) {
            ...
            log(LogLevel.INFO, `File pulled from device successfully: ${args.remotePath}`);
            return {
                content: [{ type: "text", text: pullResult.stdout }]
            };
        }
    }
    catch (error) {
        ...
        return {
            content: [{ type: "text", text: `Error pulling file: ${errorMsg}` }],
            isError: true
        };
    }
    finally {
...
// Add adb push tool
server.tool("adb_push", types_1.AdbPushSchema.shape, async (args, _extra) => {
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `Error pushing file: ${errorMsg}` }],
            isError: true
        };
    }
    finally {
...
server.tool("dump_image", types_1.AdbScreenshotSchema.shape, async (args, _extra) => {
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `Error taking screenshot: ${errorMsg}` }],
            isError: true
        };
    }
    finally {
